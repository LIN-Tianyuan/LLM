...
def calculate_loss(logit, target, pad_idx, smoothing=False):
    """
    计算模型的损失：通过函数解析下，GPT2内部如何计算损失？
    :param logit:模型的预测结果
    :param target:真实标签
    :param pad_idx:特殊-100忽略计算损失的值
    :param smoothing: 计算损失的优化方法
    :return:
    """
    if smoothing:
...
def calculate_acc(logit, labels, ignore_index=-100):
    ...
    """
    logit--->原始值的形状torch.Size([4, 226, 13317])
    labels--->原始值的形状torch.Size([4, 226])
    """
