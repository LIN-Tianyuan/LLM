from transformers import BertTokenizerFast # 导入分词器，分词工具
import pickle # 保存pkl文件的命令，将数据保存为pkl文件，方便下次读取
from tqdm import tqdm # 加载进度条，读取数据的进度条展示
import os


def data_preprocess(train_txt_path, train_pkl_path):
    """
    对原始语料进行tokenize，将每段对话处理成如下形式："[CLS]utterance1[SEP]utterance2[SEP]utterance3[SEP]"
    """

    # 初始化tokenizer，使用BertTokenizerFast从预训练的中文Bert模型（bert-base-chinese）创建一个tokenizer对象
    # tokenizer = BertTokenizerFast.from_pretrained('/Users/ligang/PycharmProjects/llm/prompt_tasks/bert-base-chinese',
    #                                               sep_token="[SEP]",
    #                                               pad_token="[PAD]",
    #                                               cls_token="[CLS]")
    # tokenizer = BertTokenizerFast.from_pretrained('/Users/ligang/PycharmProjects/llm/prompt_tasks/bert-base-chinese')
    tokenizer = BertTokenizerFast('/Users/citron/Documents/GitHub/LLM/Chatbot/vocab/vocab.txt',
                                  sep_token="[SEP]",
                                  pad_token="[PAD]",
                                  cls_token="[CLS]")

    # 词表大小
    # print(f'tokenizer.vocab_size-->{tokenizer.vocab_size}')
    # tokenizer.vocab_size-->13317

    # 特殊字符对应的id
    sep_id = tokenizer.sep_token_id  # 获取分隔符[SEP]的token ID
    cls_id = tokenizer.cls_token_id  # 获取起始符[CLS]的token ID
    # print(f'sep_id-->{sep_id}')
    # print(f'cls_id-->{cls_id}')
    # sep_id-->102
    # cls_id-->101


    # 读取训练数据集
    with open(train_txt_path, 'rb') as f:
        data = f.read().decode("utf-8")  # 以UTF-8编码读取文件内容
    # print(data)
    # # # 根据换行符区分不同的对话段落，需要区分Windows和Linux\mac环境下的换行符
    if "\r\n" in data:
        train_data = data.split("\r\n\r\n")
    else:
        train_data = data.split("\n\n")

    # print(len(train_data))  # 打印对话段落数量 30177
    # print(train_data[:1])   # ['帕金森叠加综合征的辅助治疗有些什么？\n综合治疗；康复训练；生活护理指导；低频重复经颅磁刺激治疗']
    # 开始进行tokenize
    # 保存所有的对话数据,每条数据的格式为："[CLS]seq1[SEP]seq2[SEP]seq3[SEP]"
    dialogue_len = []  # 记录所有对话tokenize分词之后的长度，用于统计中位数与均值
    dialogue_list = []  # 记录所有对话

    for index, dialogue in enumerate(tqdm(train_data)):
        # print(f'dialogue-->{dialogue}')
        """
        dialogue-->帕金森叠加综合征的辅助治疗有些什么？
        综合治疗；康复训练；生活护理指导；低频重复经颅磁刺激治疗
        """
        if "\r\n" in dialogue:
            sequences = dialogue.split("\r\n")
        else:
            sequences = dialogue.split("\n")
        # print(f'sequences--> {sequences}')
        """
        sequences--> ['帕金森叠加综合征的辅助治疗有些什么？', '综合治疗；康复训练；生活护理指导；低频重复经颅磁刺激治疗']
        """

        input_ids = [cls_id]  # 每个dialogue以[CLS]seq1[sep]seq2[sep]开头
        for sequence in sequences:
            # print(f'sequence-->{sequence}')
            """
            sequence-->帕金森叠加综合征的辅助治疗有些什么？
            """
            # print(f'tokenizer.encode(sequence, add_special_tokens=False)-->{tokenizer.encode(sequence, add_special_tokens=False)}')
            """
            tokenizer.encode(sequence, add_special_tokens=False)-->[2364, 7032, 3481, 1363, 1217, 5341, 1394, 2519, 4638, 6774, 1221, 3780, 4545, 3300, 763, 784, 720, 8043]
            """
            # print(f'tokenizer.encode(sequence)-->{tokenizer.encode(sequence)}') (会添加特殊字符)
            # break
            input_ids += tokenizer.encode(sequence, add_special_tokens=False)  # 将每个对话句子进行tokenize，并将结果拼接到input_ids列表中
            # input_ids += tokenizer.encode(sequence)  # 将每个对话句子进行tokenize，并将结果拼接到input_ids列表中
            input_ids.append(sep_id)  # 每个seq之后添加[SEP]，表示seqs会话结束
        # print(f'input_ids-->{input_ids}')
        """
        input_ids-->[101, 2364, 7032, 3481, 1363, 1217, 5341, 1394, 2519, 4638, 6774, 1221, 3780, 4545, 3300, 763, 784, 720, 8043, 102, 5341, 1394, 3780, 4545, 8039, 2434, 1908, 6378, 5298, 8039, 4495, 3833, 2844, 4415, 2900, 2193, 8039, 856, 7574, 7028, 1908, 5307, 7565, 4828, 1173, 4080, 3780, 4545, 102]
        """
        # break
        dialogue_len.append(len(input_ids))  # 将对话的tokenize后的长度添加到对话长度列表中
        dialogue_list.append(input_ids)  # 将tokenize后的对话添加到对话列表中

    # print(f'dialogue_len--->{dialogue_len}')  # 打印对话长度列表
    # print(f'dialogue_list--->{dialogue_list[:2]}')  # 打印
    """
    dialogue_list--->[[101, 2364, 7032, 3481, 1363, 1217, 5341, 1394, 2519, 4638, 6774, 1221, 3780, 4545, 3300, 763, 784, 720, 8043, 102, 5341, 1394, 3780, 4545, 8039, 2434, 1908, 6378, 5298, 8039, 4495, 3833, 2844, 4415, 2900, 2193, 8039, 856, 7574, 7028, 1908, 5307, 7565, 4828, 1173, 4080, 3780, 4545, 102], [101, 1317, 2338, 4617, 5489, 4606, 4638, 2512, 1008, 2110, 3466, 3389, 3300, 763, 784, 720, 8043, 102, 6631, 1898, 4026, 6402, 8039, 1898, 1008, 1745, 8039, 8912, 3466, 3389, 8039, 5514, 4289, 6631, 1898, 8039, 3318, 1184, 6631, 1898, 8039, 9162, 3466, 3389, 102]]
    """

    # 保存数据
    with open(train_pkl_path, "wb") as f:
        pickle.dump(dialogue_list, f)



if __name__ == '__main__':
    # train_txt_path = '../data/medical_train.txt'
    # train_pkl_path = '../data/medical_train.pkl'
    # data_preprocess(train_txt_path, train_pkl_path)
    valid_txt_path = '../data/medical_valid.txt'
    valid_pkl_path = '../data/medical_valid.pkl'
    data_preprocess(valid_txt_path, valid_pkl_path)
